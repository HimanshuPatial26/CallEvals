...
import { getLead, reassignLead } from "../api/client";
...
export default function LeadDetailModal({ leadId, agents, onClose }) {
  ...
  async function handleReassign() {
    ...
    try {
      ...
      setLead((prev) => ({ ...prev, assigned_agent_id: updated.assigned_agent_id, assignment_history: updated.assignment_history }));
    } catch (err) {
        ...
        {!lead ? (
          ...
          <>
            ...
            <p className="item-meta">
              {lead.phone || "No phone"} · {lead.source || "Unknown source"} · {STAGE_LABELS[lead.stage]}
              {lead.stage === "won" && lead.deal_size_aed != null && ` · ${lead.deal_size_aed.toLocaleString()} AED`}
            </p>

            <div className="outcome-panel">
              <label className="outcome-field">
                Reassign to
                <select value={reassignTo} onChange={(e) => setReassignTo(e.target.value)} disabled={reassigning}>
                ...
                {lead.stage_history.length === 0 ? (
                  ...
                  <ul className="insights-list">
                    {lead.stage_history
                      ...
                      .map((event, i) => (
                        <li key={i}>
                          <span className="insights-label">{STAGE_LABELS[event.stage]}</span>
                          <span className="insights-value">{fmtDateTime(event.changed_at)}</span>
                        </li>
                      ))}
                  </ul>
                )}
                      ...
                      .map((event, i) => (
                        <li key={i}>
                          <span className="insights-label">
                            {event.assigned_agent_id ? agentNameById[event.assigned_agent_id] || event.assigned_agent_id : "Unassigned"}
                          </span>
                          <span className="insights-value">{fmtDateTime(event.changed_at)}</span>
                        </li>
                      ))}
