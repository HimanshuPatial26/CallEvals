...
export default function TrendLineChart({ trend }) {
  ...
  const grid = [0, 1, 2, 3, 4].map((i) => (
    <line key={i} x1={0} x2={W} y1={(H / 4) * i} y2={(H / 4) * i} stroke="rgba(139,92,246,.13)" strokeWidth="1" />
  ));
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" style={{ width: "100%", height: 210 }}>
      ...
      {convPts.length > 1 && (
        <polyline points={polyPoints(convPts)} fill="none" stroke="#22d3ee" strokeWidth="1.6" strokeDasharray="4 4" opacity="0.85" />
      )}
      {scorePts.length > 1 && <polyline points={scoreLine} fill="none" stroke="#c084fc" strokeWidth="2.4" strokeLinejoin="round" />}
      {scorePts.map(([x, y], i) => (
        <circle key={i} cx={x} cy={y} r="2.6" fill="#0b0918" stroke="#c084fc" strokeWidth="1.4" />
      ))}
    </svg>
  );
}
